...
function wpc_enqueue_script(){
	...
	wp_enqueue_style('gt_wp_enqueue10',get_theme_file_uri('/assets/css/sweetalert.css'),array());
	wp_enqueue_style('gt_wp_enqueue6',get_theme_file_uri('/assets/css/main.css'),array());
	
if(is_page_template('compare.php')){
    wp_enqueue_style('gt_wp_enqueue9',get_theme_file_uri('/assets/css/compare-reset.css'),array());
    wp_enqueue_style('gt_wp_enqueue12',get_theme_file_uri('/assets/css/compare-style.css'),array());
    wp_enqueue_style('gt_wp_enqueue11',get_theme_file_uri('/assets/css/family.css'),array());
	...
	wp_enqueue_script('gt_wp_enqueue_script1',get_theme_file_uri('/assets/js/jquery-2.2.4.min.js'),array(),'2.4.4',false);
	...
	wp_enqueue_script('gt_wp_enqueue_script7',get_theme_file_uri('/assets/js/sweetalert.js'),array(),null,true);
	wp_enqueue_script('gt_wp_enqueue_script6',get_theme_file_uri('/assets/js/main.js'),array(),null,true);

	wp_enqueue_script('gt_wp_enqueue_script9',get_theme_file_uri('/assets/js/compare-main.js'),array(),null, true);
    wp_enqueue_script('gt_wp_enqueue_script8',get_theme_file_uri('/assets/js/compare-modernizr.js'),array(),null, true);
	...
	add_filter( 'nav_menu_link_attributes', 'add_class_to_href', 10, 2 );
		...
		function te_google_map_shortcode( $atts ) {
		    $values = shortcode_atts( array(
		        'height' => 400,
		        'width' => 600,
		        'link' => 'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3401.4069518324545!2d34.44753688484959!3d31.512981081372047!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x14fd7f6b18708d27%3A0x1f26dc8d799cd45b!2sGaza+Sky+Geeks!5e0!3m2!1sar!2s!4v1502699302178',
		    ), $atts );
		    return '<iframe src="'.$values['link'].'" width="'.$values['width'].'" height="'.$values['height'].'" frameborder="0" style="border:0" allowfullscreen></iframe>';
		}	
  ...
  // functions how to creat a axonomy for custom post 
	...
	<select name="storage">
	  <option value="4G-byte" <?php if($storage == '4G-byte'){ echo 'selected'; } ?>>4 G-byte</option>
	  <option value="8G-byte" <?php if($storage == '8G-byte'){ echo 'selected'; } ?>>8 G-byte</option>
	  <option value="16G-byte" <?php if($storage == '16G-byte'){ echo 'selected'; } ?>>16 G-byte</option>
	  <option value="24G-byte" <?php if($storage == '24G-byte'){ echo 'selected'; } ?>>24 G-byte</option>
	  <option value="32G-byte" <?php if($storage == '32G-byte'){ echo 'selected'; } ?>>32 G-byte</option>
	  <option value="64G-byte" <?php if($storage == '64G-byte'){ echo 'selected'; } ?>>64 G-byte</option>
      <option value="132G-byte" <?php if($storage == '132G-byte'){ echo 'selected'; } ?>>132G-byte</option>
   </select>
   </fieldset>
    <fieldset>
   ...
   <fieldset>
	...
	<select name="company">
	  <option value="m-apple" <?php if($company == 'm-apple'){ echo 'selected'; } ?>>m-Apple</option>
	  <option value="l-apple" <?php if($company == 'l-apple'){ echo 'selected'; } ?>>l-Apple</option>
	  <option value="t-apple" <?php if($company == 't-apple'){ echo 'selected'; } ?>>t-Apple</option>
	  <option value="m-summsung" <?php if($company == 'm-summsung'){ echo 'selected'; } ?>>m-Summsung</option>
	  <option value="l-summsung" <?php if($company == 'l-summsung'){ echo 'selected'; } ?>>l-Summsung</option>
	  <option value="t-summsung" <?php if($company == 't-summsung'){ echo 'selected'; } ?>>t-Summsung</option>
	  ...
	  <option value="t-lenovo" <?php if($company == 't-lenovo'){ echo 'selected'; } ?>>t-Lenovo</option>
	  <option value="l-lenovo" <?php if($company == 'l-lenovo'){ echo 'selected'; } ?>>l-Lenovo</option>
	  ...
	  <option value="l-others" <?php if($company == 'l-others'){ echo 'selected'; } ?>>l-Others</option>
	  <option value="t-others" <?php if($company == 'others'){ echo 'selected'; } ?>>t-Others</option>
	  <option value="m-others" <?php if($company == 'others'){ echo 'selected'; } ?>>m-Others</option>
   </select>
   </fieldset>	
    <fieldset>
...
function count_posts_by_company_tag($arg){
    ...
    while ($products->have_posts()){
        ...
        $post_meta_value = get_post_meta(get_the_ID(),'company',true);

        if (strpos( $post_meta_value, 'msi' ) !== false){
            $msi++;
        }
        elseif (strpos( $post_meta_value, 'apple' ) !== false){
            $apple++;
        }
        elseif (strpos( $post_meta_value, 'nokia' ) !== false){
            $nokia++;
        }
        elseif (strpos( $post_meta_value, 'summsung' ) !== false){
            $summsung++;
        }
        elseif (strpos( $post_meta_value, 'htc' ) !== false){
            $htc++;
        }
        elseif (strpos( $post_meta_value, 'lg' ) !== false){
            $lg++;
        }
        elseif (strpos( $post_meta_value, 'lenovo' ) !== false){
            $lenovo++;
        }
        elseif (strpos( $post_meta_value, 'asus' ) !== false){
            $asus++;
        }
        elseif (strpos( $post_meta_value, 'hp' ) !== false){
            $hp++;
        }
        elseif (strpos( $post_meta_value, 'sony' ) !== false){
            $sony++;
        }
        else{
            $others++;
